...
runtime チェックで正常に計測したバイアスは
`vehicle/.calibration/<VEHICLE_ID>/imu_bias.yaml` にも保存し、次の提出物の展開時に適用します。
ID は環境変数 → リポジトリ直下の `.env` の順で取得し、未設定・不正な場合は失敗します。
ノイズ超過や計測不能では保存元を変更しません。
