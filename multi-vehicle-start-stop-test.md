...
| `next_state(state, event, observations, joy) -> ManagerState` | モード遷移の決定 |
| `status(state, observations, joy) -> Status` | モード・各遷移の可否・不許可の理由・警告 |
...
**自動化するのは L1 だけ。** ROS を起動するテストと実機 driver を使うテストは自動化せず人力で行う（第12章）。
...
VEHICLES = ("A2", "A3", "A6", "A7")
```

観測のビルダも conftest に置く。`all_stopped()` は「4台とも停止・emergency 済み・テレメトリ新鮮」を返し、`all_stopped(A3=dict(velocity=0.5))` のように車両単位で崩す。
...
その代わり、**それらでしか検証できない前提を第13章のチェックリストへ明示的に移す**。ロジックテストは「決めた通りに動くか」しか見ておらず、「決めたことが正しいか」は見ていない。この区別を失うと、全ケース緑のまま設計の前提が間違っている状態になる。
...
| ID | 確認内容 |
| --- | --- |
| C-01 | manager 起動直後、どの車両にも joy が届いていない（`ros2 topic hz` で0件）。GUI に4台の状態が表示される |
...
| C-14 | 4台接続した状態でテレメトリの受信間隔を観測し、`telemetry_timeout_s = 1.0` が誤検出しない余裕を持つことを確認する |
